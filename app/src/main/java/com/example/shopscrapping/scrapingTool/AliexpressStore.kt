...
suspend fun AliexpressFetcher(urlToScrape: String): ScrapState =
...
//                    append(HttpHeaders.Cookie,cookieValue)
            ...
            ScrapState(url = urlToScrape, url_refered = urlToScrape, store = Store.ALIEXPRESS, product = scrapedProduct)
    ...
    if (subProductos != null) {
        subProductos.forEach { element ->
            var subProducto = SubProduct(identifier = element.string("skuIdStr")?:"",
                price = element.obj("skuVal")?.obj("skuActivityAmount")?.float("value")?:0.0f,
                src_image = "",
                aditional_title = ""
            )
            currency = element.obj("skuVal")?.obj("skuActivityAmount")?.string("currency") ?: currency
...
//            subProducto.reference = referencia TODO
            ...
            array<JsonObject>("productSKUPropertyList")?.forEachIndexed  { index, elementoRef ->
                elementoRef.array<JsonObject>("skuPropertyValues")?.forEach { element1 ->
                    if (element1.int("propertyValueId") == referencia.get(index).toInt())
                    {
